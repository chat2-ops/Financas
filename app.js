...
$('#cardFilterBtn').addEventListener('click', () => { cardOnly = !cardOnly; $('#cardFilterBtn').classList.toggle('active', cardOnly); $('#cardFilterBtn').setAttribute('aria-pressed', String(cardOnly)); render(); if (cardOnly) setTimeout(() => $('#cardSummary').scrollIntoView({ behavior: 'smooth', block: 'center' }), 0); });
